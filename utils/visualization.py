...
class disp_error_image_func(Function):
    def forward(self, D_est_tensor, D_gt_tensor, abs_thres=3., rel_thres=0.05, dilate_radius=1):
        ...
        error_image_notag=error_image.copy()
        for i in range(cols.shape[0]):
            distance = 20
            error_image[:, :10, i * distance:(i + 1) * distance, :] = cols[i, 2:]

        return torch.from_numpy(np.ascontiguousarray(error_image.transpose([0, 3, 1, 2]))),torch.from_numpy(np.ascontiguousarray(error_image_notag.transpose([0, 3, 1, 2])))
